...
import { validateResponse, TelemetryLatestResponseSchema, DevicesResponseSchema, AlarmsResponseSchema, GenericResponseSchema, RulesResponseSchema, SchedulesResponseSchema, UsersResponseSchema, ConfigResponseSchema } from '@/lib/schemas';

// ── Helpers ──

/** T3-FE-004: Parse JSON response and throw on HTTP errors */
async function parseJsonOrThrow(res: Response): Promise<unknown> {
  if (!res.ok) {
    let detail = '';
    try { const e = await res.json(); detail = e?.error || e?.message || ''; } catch {}
 ...
 * P2-CSRF-01: Uses CSRF token.
 * T3-FE-005: Removed client-side CSRF rotation; server rotates via Set-Cookie.
 */
 ...
 * P2-CSRF-01: Uses CSRF token.
 * T3-FE-005: Removed client-side CSRF rotation.
 */
...
export async function fetchRules(): Promise<RulesResponse> {
  ...
  const data = await parseJsonOrThrow(res);
  return validateResponse(RulesResponseSchema, data);
}
...
export async function fetchSchedules(): Promise<SchedulesResponse> {
  ...
  const data = await parseJsonOrThrow(res);
  return validateResponse(SchedulesResponseSchema, data);
}
...
export async function fetchUsers(): Promise<UsersResponse> {
  ...
  const data = await parseJsonOrThrow(res);
  return validateResponse(UsersResponseSchema, data);
}
...
export async function fetchConfig(): Promise<ConfigResponse> {
  ...
  const data = await parseJsonOrThrow(res);
  return validateResponse(ConfigResponseSchema, data);
}
