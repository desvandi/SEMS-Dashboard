...
bool SOC_Calculator::saveToEEPROM() {
    // T3-FW-016: Simple mutex pattern to prevent concurrent EEPROM writes
    static bool eepromBusy = false;
    ...
    // T3-FW-008: Only save if SOC changed significantly (reduce EEPROM wear)
    float prevSOC = 0.0f;
    float prevVoltage = 0.0f;
    EEPROM.get(EEPROM_ADDR_SOC, prevSOC);
    EEPROM.get(EEPROM_ADDR_CAL_V, prevVoltage);
    if (fabsf(_data.socPercent - prevSOC) < 0.5f && fabsf(_data.lastVoltage - prevVoltage) < 0.1f) {
        // No significant change — skip write
        eepromBusy = false;
        return true;
    }
