...
export function AnalyticsChartsSection({ analytics, colors }: Props) {
  // T3-FE-026: Use unique IDs for SVG gradients to prevent collisions
  const uid = useId().replace(/:/g, '');
  const powerInGradId = `powerInGrad-${uid}`;
  const powerOutGradId = `powerOutGrad-${uid}`;
  const socRangeGradId = `socRangeGrad-${uid}`;
      ...
      >
        ...
        <ResponsiveContainer width="100%" height={280}>
          <AreaChart data={analytics.dailyData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
            <defs>
              <linearGradient id={socRangeGradId} x1="0" y1="0" x2="0" y2="1">
