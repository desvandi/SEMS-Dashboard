    ...
    try {
      ...
      for (var i = 1; i < data.length; i++) {
        if (String(data[i][colId]) === String(payload.id)) {
          // T3-BE-014: Validate device state is 0 or 1
          var s = parseInt(payload.state);
          if (s !== 0 && s !== 1) {
            return { success: false, error: 'State must be 0 or 1', code: 400 };
          }
          // Update state and timestamp
          sheet.getRange(i + 1, colState + 1).setValue(s);
...
function handleDevicesControl_(payload, reqHeaders) {
  try {
    ...
    if (payload.pwm_value !== undefined) {
      ...
      // BE-009 FIX: pwm_value is validated but NOT stored to sheet.
      // The devices sheet only has a 'state' column (0/1). PWM values are not
      // persisted because there is no dedicated column. ESP32 must track PWM
      // state internally or it resets to 0 on next telemetry cycle.
    }
