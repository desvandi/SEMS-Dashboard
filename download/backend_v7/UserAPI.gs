...
function handleUsersChangePassword_(payload, headers) {
  try {
    ...
    try {
      ...
      // BE-012 FIX: Re-read sheet data inside lock to prevent TOCTOU race.
      // Previously, data was read outside lock (line ~152) but write happened inside
      // lock (line ~232), creating a window where another request could modify
      // the user row between read and write.
      var lockData = sheet.getDataRange().getValues();
      var lockHeaders = lockData[0];
      var lockColPassHash = lockHeaders.indexOf('password_hash');
      var lockColMustChangePass = lockHeaders.indexOf('must_change_password');
      var lockColId = lockHeaders.indexOf('id');
      var lockColUsername = lockHeaders.indexOf('username');

      // Re-find user row inside lock (row may have shifted)
      var lockUserRow = -1;
      for (var li = 1; li < lockData.length; li++) {
        if (String(lockData[li][lockColUsername]).trim().toLowerCase() === username) {
          lockUserRow = li + 1; // 1-based
          break;
        }
      }
      if (lockUserRow === -1) {
      ...
      sheet.getRange(lockUserRow, lockColPassHash + 1).setValue(newHash);

      // Clear must_change_password flag
      if (lockColMustChangePass !== -1) {
        sheet.getRange(lockUserRow, lockColMustChangePass + 1).setValue(false);
      }

      // Invalidate the user's existing sessions
      var userId = String(lockData[lockUserRow - 1][lockColId]);
      ...
      for (var i = 1; i < data.length; i++) {
        if (String(data[i][colId]) === String(payload.id)) {
          ...
          if (payload.username) {
            ...
            // BE-010 FIX: Invalidate the user's sessions when username changes,
            // since session data contains the old username.
