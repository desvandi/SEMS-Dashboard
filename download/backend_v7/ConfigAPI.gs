...
function handleConfigUpdate_(payload, reqHeaders) {
  try {
    ...
    // T3-BE-017: Numeric validation map for config values
    var NUMERIC_VALIDATION = {
      'voltage_calibration': { min: 0.1, max: 10 },
      'current_calibration': { min: 0.1, max: 10 },
      'soc_low_threshold': { min: 5, max: 50 },
      'soc_critical_threshold': { min: 1, max: 30 },
      'max_load_current': { min: 1, max: 200 },
      'data_retention_days': { min: 7, max: 365 },
      'alarm_retention_days': { min: 7, max: 365 },
      'overvoltage_threshold': { min: 20, max: 35 },
      'overload_current': { min: 1, max: 200 },
      'low_battery_voltage': { min: 10, max: 28 }
    };
    ...
    try {
      ...
      // BE-011 FIX: Use global SENSITIVE_KEYS constant instead of incomplete local array
      var sensitiveKeys = SENSITIVE_KEYS;
      ...
      for (var u = 0; u < updates.length; u++) {
        ...
        // T3-BE-017: Validate numeric ranges for known numeric config keys
        if (NUMERIC_VALIDATION[update.key]) {
          var numVal = parseFloat(update.value);
          if (isNaN(numVal)) {
            return { success: false, error: 'Value for "' + update.key + '" must be a number', code: 400 };
          }
          var bounds = NUMERIC_VALIDATION[update.key];
          if (numVal < bounds.min || numVal > bounds.max) {
            return { success: false, error: 'Value for "' + update.key + '" must be between ' + bounds.min + ' and ' + bounds.max, code: 400 };
          }
        }
        ...
        for (var i = 1; i < data.length; i++) {
          if (String(data[i][colKey]) === String(update.key)) {
            ...
            applied.push({
              key: update.key,
              // T3-BE-009 / BE-002 FIX: Mask sensitive values in update response;
              // use SENSITIVE_KEYS.indexOf() since isSensitive was undefined in this scope
              value: SENSITIVE_KEYS.indexOf(update.key) !== -1 ? '***' : String(update.value),
              updated_at: now
            });
            break;
          }
        ...
        if (!found) {
          ...
          applied.push({
            key: update.key,
            // T3-BE-009: Mask sensitive values in update response
            value: sensitiveKeys.indexOf(update.key) !== -1 ? '***' : String(update.value),
            updated_at: now,
            created: true
          });
        }
