...
function getAlarmPlainBody_(alarm) {
  ...
  // T3-BE-022: Sanitize system name for plain text email (remove newlines, HTML tags)
  var systemName = (getConfigValue_('system_name') || 'SEMS - Smart Energy Management System')
...
function incrementDailyEmailCount_() {
  ...
  // T3-BE-012: Calculate end-of-day TTL in configured timezone
  // (not server timezone which may differ)
  var tz = getTimezone_();
  var todayStr = Utilities.formatDate(new Date(), tz, 'yyyy-MM-dd');
  var tomorrow = new Date(todayStr + 'T00:00:00');
  tomorrow.setDate(tomorrow.getDate() + 1);
  var ttlSeconds = Math.max(1, Math.round((tomorrow.getTime() - Date.now()) / 1000));
