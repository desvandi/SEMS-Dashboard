...
export default function AlarmsPage() {
  ...
  // T3-FE-015: Reset displayCount when filters change
  useEffect(() => { setDisplayCount(20); }, [severityFilter, statusFilter]);
  ...
  // ---- Acknowledge single ----
  // FE-050 FIX: Accept alarm object directly instead of display index from filtered list
  const handleAck = async (alarm: Alarm) => {
    const idx = alarms.findIndex(a => a.id === alarm.id);
    if (idx === -1) return;
    setAckingIdx(idx);
    try {
      // FE-050 FIX: Use alarm parameter directly instead of alarms[idx]
      if (!alarm) return;
                        ...
                        >
                          ...
                          <div className="flex items-center gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
                            {!alarm.acknowledged && (
                              ...
                              >
                                {ackingIdx !== null && alarms[ackingIdx]?.id === alarm.id ? (
                                  <Loader2 className="w-3.5 h-3.5 animate-spin" />
                                ) : (
